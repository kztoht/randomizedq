// 全20パッセージを配列に保存（ここではサンプルとして、Passage 1〜20 のすべてのテキストをオブジェクト形式で定義）
const passagesArray = [
  ...
  { id: 'Passage 2', text: "Every morning, I wake up at 6:30 and make [a] simple breakfast. I usually have [an] egg, toast, and milk. After breakfast, I take [a] shower and get dressed for school. I leave home at 7:30 and walk to [the] station. On [the] train, I read [a] book or listen to music. When I arrive at school, I greet [the] guard at the gate and go to my classroom. Before class starts, I review [ ] notes from the day before. It's [a] quiet time that helps me focus. Then, [the] bell rings and the lessons begin." },
  ...
  { id: 'Passage 17', text: "My father took me to [a] baseball game. We watched [the] game [ ] last weekend. We bought [a] ticket and entered [the] stadium. The seats were close to [the] field, so we could see the players clearly. I ate [a] hot dog and drank a soda. Everyone cheered when our team scored. A man behind us dropped [a] bag of popcorn, and [a] bird came down to eat it. Between innings, we went to [the] shop to buy souvenirs. I got a small keychain. Watching live sports is a great way to spend time with family." },
...
function parsePassage(text) {
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 0) {
      ...
      segments.push(null); // null の場所に後でセレクト要素を挿入する
    }
  }
...
// 現在のパッセージの情報
let currentParsedPassage = null;
let currentUserAnswers = [];

// ランダムにパッセージを選び、画面に表示する関数
function loadRandomPassage() {
  const randomIndex = Math.floor(Math.random() * passagesArray.length);
  const passageObj = passagesArray[randomIndex];
  currentParsedPassage = parsePassage(passageObj.text);
  currentUserAnswers = Array(currentParsedPassage.answers.length).fill("");
  
  const passageDiv = document.getElementById("passage");
  passageDiv.innerHTML = ""; // クリア
  
  currentParsedPassage.segments.forEach((segment, idx) => {
    if (segment === null) {
      // セレクトボックスを作成
      ...
      selectEl.addEventListener("change", (e) => {
        ...
        currentUserAnswers[blankIndex] = e.target.value;
      });
      passageDiv.appendChild(selectEl);
    } else {
      // テキスト部分を追加
...
function submitAnswers() {
  let score = 0;
  currentParsedPassage.answers.forEach((correct, idx) => {
    if (currentUserAnswers[idx] === correct) {
      score++;
    }
  });
  document.getElementById("score-display").textContent = `Your Score: ${score} / ${currentParsedPassage.answers.length}`;
  // 管理者モードが有効なら、統計データを更新
  if (statsActive) {
    totalScore += score;
    totalQuestions += currentParsedPassage.answers.length;
    updateStatsDisplay();
  }
  // 次のパッセージを読み込むための Next ボタンを表示
...
function updateStatsDisplay() {
  ...
  const rate = (totalScore / totalQuestions * 100).toFixed(2);
  statsDiv.textContent = `Overall Correct Rate: ${rate}%`;
}

// イベントリスナーの登録
document.getElementById("submit-btn").addEventListener("click", submitAnswers);
document.getElementById("next-btn").addEventListener("click", loadRandomPassage);
document.getElementById("start-stats-btn").addEventListener("click", () => {
  statsActive = true;
  totalScore = 0;
  totalQuestions = 0;
  updateStatsDisplay();
...
// 初回ロード時にパッセージを表示
window.onload = loadRandomPassage;
